...
function readDateQueryParam(query, name) {
...
function parseUserNotification(payload) {
...
function parseDeliveryAck(payload) {
    if (!isObjectPayload(payload)) {
        return null;
    }
...
function startNotificationMqttListener() {
    const client = mqtt.connect(mqttUrl, { reconnectPeriod: 1000 });
    client.on('connect', () => {
        console.log(`analytics-pipeline connected to MQTT broker at ${mqttUrl}`);
        client.subscribe([userNotificationTopic, notificationAckTopic], (error) => {
            if (error) {
                console.error(`analytics-pipeline failed to subscribe MQTT topics: ${error.message}`);
                return;
            }
            console.log(`analytics-pipeline subscribed to MQTT topics: ${userNotificationTopic}, ${notificationAckTopic}`);
        });
    });
...
app.get('/analytics/orders/trend', (req, res) => {
...
app.get('/analytics/customers/tier-distribution', (req, res) => {
